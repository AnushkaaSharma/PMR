var EMRContractContract = artifacts.require("./EMRContractContract.sol");

var Web3 = require('web3');
var web3 = new Web3(new Web3.providers.HttpProvider("http://localhost:7545"));

contract('Initialize EMRContract Smart-Contract.', function (accounts) {
    ...
    it("Initialized the EMRContract project.", function () {
        return EMRContract.deployed().then(function (instance) {
            EMRContractInstance = instance;
            return EMRContractInstance.version();
        }).then((version) => {
    ...
    it("Kill the app.", function () {
        return EMRContract.deployed().then(function (instance) {
            EMRContractInstance = instance;
            return EMRContractInstance.kill({ from: admin1 });
        }).then(assert.fail).catch(function (error) {
            assert(error.message.indexOf('revert') >= 0, 'msg.value must equal number of tokens in wei');
            //return EMRContractInstance.kill({from: accounts[0]});
        })
    })
    ...
    it("Add administrator.", function () {
        return EMRContract.deployed().then(function (instance) {
            EMRContractInstance = instance;
            return EMRContractInstance.SetAdministrator(admin2, { from: admin1 });
        }).then(assert.fail).catch(function (error) {
            assert(error.message.indexOf('revert') >= 0, 'msg.value must equal number of tokens in wei');
            return EMRContractInstance.SetAdministrator(admin1, { from: owner });
        }).then((receipt) => {
            //console.log(receipt)
            return EMRContractInstance.administrator(admin1);
        }).then((result) => {
            assert.equal(result, true, 'Account Hash is correct');
            return EMRContractInstance.SetAdministrator(admin2, { from: owner });
        }).then((receipt) => {
            //console.log(receipt)
            return EMRContractInstance.administrator(admin2);
        }).then((result) => {
            assert.equal(result, true, 'Account Hash is correct');
        })
    })
    ...
    it("Sign Up Patient.", function () {
        ...
        return EMRContract.deployed().then(function (instance) {
            ...
            /*return EMRContractInstance.SignupPatient(_fullname, _dob, _sex,
            ...
            /*return EMRContractInstance.PatientUpdate(_fullname + increment, _dob, _sex + increment,
            ...
            return EMRContractInstance.PatientUpdate(_fullname + increment, _dob, _sex + increment,
                ...
                assert(error.message.indexOf('revert') >= 0, 'msg.value must equal number of tokens in wei');
                return EMRContractInstance.SignupPatient(_fullname + "p2", _dob, _sex,
                    _marital, _email, ReportDocsArray, { from: patient2 });


        }).then((receipt) => {
                return EMRContractInstance.patients(patient2);
        }).then((results) => {
                assert.equal(results[0], patient2, 'Account Hash is correct');
                return EMRContractInstance.SignupPatient(_fullname + "p3", _dob, _sex,
                    _marital, _email, ReportDocsArray, { from: patient3 });


            }).then((receipt) => {
                return EMRContractInstance.patients(patient3);
            }).then((results) => {
    ...
    it("Add Appointment.", function () {
        ...
        return EMRContract.deployed().then(function (instance) {
            EMRContractInstance = instance;
            return EMRContractInstance.AppointmentAdd(_datetime, AppointmentStat, _remark, { from: patient1 });
        }).then((reply) => {

            return EMRContractInstance.AppointmentAdd(_datetime, AppointmentStat, _remark, { from: patient4 });
        }).then(assert.fail).catch(function (error) {
            assert(error.message.indexOf('revert') >= 0, 'msg.value must equal number of tokens in wei');
            return EMRContractInstance.AppointmentAdd(new Date("2018-09-20").getTime(), AppointmentStat, _remark, { from: patient1 });
        }).then((receipt) => {
            //console.log(receipt);
            ...
            return EMRContractInstance.AppointmentIndex()
        }).then((results) => {
            //console.log(results);
            AppointmentIndex = results.toNumber();
            return EMRContractInstance.appointments(AppointmentIndex);
        }).then((results) => {
            //console.log(results);
            return EMRContractInstance.AppointmentUpdate(AppointmentIndex, 1, { from: admin1 });
        }).then((receipt) => {
            assert.equal(receipt.logs.length, 1, 'triggers one event');
    ...
    it("Add medical report", function () {
        ...
        return EMRContract.deployed().then(function (instance) {
            EMRContractInstance = instance;
            return EMRContractInstance.MedicalReportAdd(docname + "1", docpath + "1", docdesc + "1", { from: patient1 });
        }).then((reply) => {
            //console.log(reply);
            return EMRContractInstance.MedicalReportAdd(docname + "2", docpath + "2", docdesc + "2", { from: patient1 });
        }).then((reply) => {
            //console.log(reply);
            return EMRContractInstance.medicalreports(1);
        }).then((reply) => {
            console.log(reply);
            return EMRContractInstance.MedicalReportGet(patient1);
        }).then((reply) => {
